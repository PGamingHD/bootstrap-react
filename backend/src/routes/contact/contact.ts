...
router.get("/count", async (req: Request, res: Response): Promise<Response<any, Record<string, any>>> => {
    if (!process.env.MYSQL_PORT) return res.send({message: 'failed', response: 'Failed to find MySQL Connection Port!'});
...
router.put("/updateRead", async (req: Request, res: Response): Promise<Response<any, Record<string, any>>> => {
    if (!process.env.MYSQL_PORT) return res.send({message: 'failed', response: 'Failed to find MySQL Connection Port!'});
    ...
    return res
        .json({ message: "success", response: "Successfully updated read on ID!" })
        .status(200);
});

router.delete("/deleteContact/:contactId", validateAuth, async (req: Request, res: Response): Promise<Response<any, Record<string, any>>> => {
    if (!process.env.MYSQL_PORT) return res.send({message: 'failed', response: 'Failed to find MySQL Connection Port!'});
